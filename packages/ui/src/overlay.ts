...
export function drawDebugGeometryOverlay(
  ...
  const sourceWidth = Math.max(1, frameSize.width);
  const sourceHeight = Math.max(1, frameSize.height);
  const scaleX = video.clientWidth / sourceWidth;
  const scaleY = video.clientHeight / sourceHeight;

  drawDebugGeometry(ctx, scaleX, scaleY, debugOverlay);
}
