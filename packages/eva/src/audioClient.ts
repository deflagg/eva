...
export interface AudioClientHandlers {
  onOpen?: () => void;
  onClose?: () => void;
...
export function createAudioClient(options: AudioClientOptions): AudioClient {
  ...
  const connectSocket = (): void => {
    ...
    ws.on('message', (data, isBinary) => {
      if (isBinary) {
        handlers.onInvalidMessage?.('<unexpected binary message from Audio Runtime>');
        return;
      }
