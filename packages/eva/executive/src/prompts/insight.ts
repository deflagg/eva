...
export function buildInsightSystemPrompt(maxFrames: number, ttsStyle: 'clean' | 'spicy' = 'clean'): string {
  ...
  return [
    ...
    '- presence.preson_present: boolean (true when one or more people are visible, else false).',
    '- presence.person_facing_me: boolean (true only when a visible person is facing toward the camera/viewpoint).',
    '',
    ...
    '',
    'PRESENCE FIELD CONSISTENCY (HARD RULE):',
    '- Always include both booleans: presence.preson_present and presence.person_facing_me.',
    '- If no person is visible, set preson_present=false and person_facing_me=false.',
    '- person_facing_me=true is only valid when preson_present=true.',
    '',
    ...
    '- If an object fell/moved/spun, react briefly and lightly check safety without alarmism.',
    ...
    '- If uncertain, be explicit and avoid over-claiming.',
