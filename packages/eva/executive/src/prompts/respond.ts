...
export function buildRespondSystemPrompt(input: RespondSystemPromptInput): string {
  ...
  return [
    ...
    '- Treat live wm_event lines as environment-state context, not user-facing telemetry output.',
    '- Default to natural language summaries of what is happening; avoid dumping raw telemetry.',
    '- Do not repeat raw fields (for example track IDs, speed metrics, or detector key/value payloads) unless the user explicitly asks for low-level details or debugging output.',
