...
def _read_server_host() -> str:
    ...
    if not isinstance(host, str) or not host.strip():
        raise RuntimeError("Vision config error: server.host must be a non-empty string")
...
def _read_server_port() -> int:
    ...
    if isinstance(raw_port, bool):
        raise RuntimeError("Vision config error: server.port must be an integer in range 1..65535")
